...
test('log out', async ({ page }) => {
  ...
  await page.getByRole('menuitem', { name: 'khulnasoft e2e' }).last().hover();
...
test('switch organization', async ({ page }) => {
  ...
  const switcher = page.getByRole('button', { name: 'khulnasoft-e2e' });

  await switcher.click();
  await page.getByRole('option', { name: 'khulnasoft-e2e-2' }).click();

  await expect(switcher).toHaveAccessibleName('khulnasoft-e2e-2');
});
