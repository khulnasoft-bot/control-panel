...
function useRegisterExternalNavigationCommands() {
  ...
  useMount(() => {
    defaultItems.add({
      label: 'Go to Snipkit website',
      description: 'Open www.snipkit.com',
      keywords: ['website', 'snipkit'],
      execute: () => void window.open('http://www.snipkit.com'),
    });

    defaultItems.add({
      label: "View Snipkit's pricing",
      description: 'Open snipkit.com/pricing',
      keywords: ['pricing', 'cost', 'money'],
      execute: () => void window.open('http://snipkit.com/pricing'),
    });

    defaultItems.add({
      label: 'View all one-click applications',
      description: 'Open snipkit.com/deploy',
      keywords: ['deploy', 'example', 'one-click'],
      execute: () => void window.open('http://snipkit.com/deploy'),
    });

    defaultItems.add({
      label: 'View Snipkit tutorials',
      description: 'Open snipkit.com/tutorials',
      keywords: ['tutorials', 'help'],
      execute: () => void window.open('http://snipkit.com/tutorials'),
    });

    defaultItems.add({
      label: "View Snipkit's changelog",
      description: 'Open snipkit.com/changelog',
      keywords: ['changelog'],
      execute: () => void window.open('http://snipkit.com/changelog'),
    });

    defaultItems.add({
      label: "View Snipkit's blog",
      description: 'Open snipkit.com/blog',
      keywords: ['blog'],
      execute: () => void window.open('http://snipkit.com/blog'),
    });

    defaultItems.add({
      label: 'Go to Snipkit documentation',
      description: 'Open snipkit.com/docs',
      weight: 1,
      keywords: ['documentation', 'docs', 'help', 'manual'],
      execute: () => void window.open('http://snipkit.com/docs'),
    });

    defaultItems.add({
      label: 'Go to Snipkit API documentation',
      description: 'Open developer.snipkit.com',
      keywords: ['documentation', 'docs', 'help', 'api', 'manual'],
      execute: () => void window.open('http://developer.snipkit.com'),
    });

    defaultItems.add({
      label: 'Go to the Snipkit community platform',
      description: 'Open community.snipkit.com',
      keywords: ['community', 'discourse', 'help'],
      weight: 1,
      execute: () => void window.open('http://community.snipkit.com'),
    });

    defaultItems.add({
      label: 'Open a feature request or give feedback',
      description: 'Open feedback.snipkit.com',
      keywords: ['feedback', 'canny', 'feature', 'idea', 'improvement'],
      execute: () => void window.open('http://feedback.snipkit.com'),
    });

    defaultItems.add({
      label: "Open the platform's status page",
      description: 'Open status.snipkit.com',
      keywords: ['status', 'instatus', 'platform', 'outage', 'uptime'],
      execute: () => void window.open('http://status.snipkit.com'),
    });
  });
...
function useRegisterAccountCommands() {
  ...
  useMount(() => {
    defaultItems.add({
      ...
      description: 'Create a new Snipkit organization',
    ...
    defaultItems.add({
      label: 'Log out',
      description: 'Sign out from the Snipkit control panel',
...
function useRegisterMiscCommands() {
  ...
  useMount(() => {
    ...
    defaultItems.add({
      ...
      description: 'View the source code of the Snipkit control panel on GitHub',
      keywords: ['code', 'snipkit'],
      execute: () => void window.open('https://github.com/snipkit/control-panel'),
    });

    if (organization?.plan !== 'hobby') {
      defaultItems.add({
        label: 'Contact Snipkit support',
