import { Code as BaseCode, Button, CodeLang, Spinner, TabButton, TabButtons } from '@design-system';
...
const content = {
  python: {
    install: 'pip install khulnasoft-sdk',
    run: 'python main.py',
    code: `
# main.py
...
`.trim(),
  ...
  javascript: {
    install: 'npm install @khulnasoft.com/sandbox-sdk',
    run: 'node main.js',
    code: `
// main.js
import { Sandbox } from '@khulnasoft.com/sandbox-sdk';
...
function ApiTokenCode() {
  ...
  return (
    <Code
      lang="shell"
      prefix="$ "
      value={`export KHULNASOFT_API_TOKEN=${mutation.data?.credential?.token ?? ''}`}
      end={end()}
    />
  );
}
