...
import { Alert, Button, ButtonMenuItem, MenuItem } from '@snipkit/design-system';
...
function EditAppDialog({ app }: { app: App }) {
  ...
  const snipkitDomain = app.domains.find(hasProperty('type', 'AUTOASSIGNED'));
  const [subdomain = '', domainSuffix] = splitDomain(snipkitDomain);
  ...
  const mutation = useMutation({
    async mutationFn(values: FormValues<typeof form>) {
      ...
      if (snipkitDomain && values.subdomain !== subdomain) {
        promises.push(
          api.editDomain({
            token,
            path: { id: snipkitDomain.id },
            query: {},
